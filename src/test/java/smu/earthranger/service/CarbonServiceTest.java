...
class CarbonServiceTest {
    ...
    public void getRank(){
        ...
        //when
        List<MemberRankingResponseDto> byPermission = rankService.getByPermission(save.getId(),"all",pageable);

        //then
        Assertions.assertThat(byPermission.get(0).getMemberId()).isEqualTo(member.getId());

        for (MemberRankingResponseDto userRankingResponseDto : byPermission) {
